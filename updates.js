...
this.extraEndPoint = `https://api.themoviedb.org/3/tv/${this.data.id}/season/${this.season}/episode/${this.episode}?api_key=${TheMovieDB_APIKey}`
  ...
  async getDetails() {
    try {
      function fetchWhenInView(element, url) {
...
}  fetchWhenInView(this)
      async function fetchInfo(element){
        const response = await fetch(element.endPoint, {
        mode: 'cors'
      })
      if (response.ok) {
        const jsonResponse = await response.json();
        element.populateCard(jsonResponse)
      } else {
        const jsonError = await response.json();
        element.populateError(jsonError)
      }
      }
